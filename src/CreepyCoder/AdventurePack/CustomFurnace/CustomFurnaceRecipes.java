...
public class CustomFurnaceRecipes {
	...
	public void loadRecipes(Plugin plugin, DataManager CustomFurnaceData) {
		...
		this.listRecipes = (List<String>) CustomFurnaceData.getConfig().getList("FurnaceRecipeList.key");
		...
		for(Iterator<String> i = this.listRecipes.iterator(); i.hasNext(); ) {
			...
			Bukkit.addRecipe(new FurnaceRecipe(new NamespacedKey(plugin, key), this.result , this.source, this.data, this.experience, this.cookingTime));
		}
	}
